...
### Это подзадание поможет тебе, Артем, познакомиться с базовыми операциями на языке С, такие как ввод, вывод и арифметика
...
>Памятка git:
>- `git init` — В начале работы.
>- `git add`, `git commit` — Когда добавил изменения. Не забыть добавить понятное название коммита.
...
>- На github у тебя должно быть две ветки: основная `main` или `master` - находится текущий репозиторий с невыполненым заданием и ветка `develop` - в ней должно быть выполненое задание.
...
в таком случае должен измениться на - "Hello, \<name>!"
